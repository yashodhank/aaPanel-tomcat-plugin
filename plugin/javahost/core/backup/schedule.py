...
def list_schedules() -> List[Dict]:
    ...
    return [{"app": a, "cron": data[a].get("cron"),
             "remote": bool(data[a].get("remote")),
             "keep": int(data[a].get("keep") or 0)} for a in sorted(data)]


def set_schedule(app: str, cron_expr: str, remote: bool = False, keep: int = 7) -> Dict:
    app = validate.identifier(app, "app")
    cron = validate_cron(cron_expr)
    keep = max(0, int(keep))
    data = _read()
    data[app] = {"cron": cron, "remote": bool(remote), "keep": keep}
    _write(data)
    _regenerate(data)
    return {"app": app, "cron": cron, "remote": bool(remote), "keep": keep}
