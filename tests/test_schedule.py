...
def test_set_list_remove_round_trip(sched):
    schedule.set_schedule("app1", "0 2 * * *", remote=True, keep=5)
    schedule.set_schedule("app2", "0 3 * * 0", remote=False, keep=3)
    items = {s["app"]: s for s in schedule.list_schedules()}
    assert items["app1"]["cron"] == "0 2 * * *" and items["app1"]["remote"] is True and items["app1"]["keep"] == 5
    assert items["app2"]["remote"] is False
    # cron.d rendered with both apps + the runner + retention/remote flags
    body = open(schedule.CRON_PATH).read()
    assert "--app app1 --remote --keep 5" in body
    assert "--app app2 --keep 3" in body
    assert body.startswith("# Managed by JavaHost")
    assert " root " in body  # cron.d requires a user field
    # idempotent update
    schedule.set_schedule("app1", "0 4 * * *", remote=False, keep=10)
    items = {s["app"]: s for s in schedule.list_schedules()}
    assert items["app1"]["cron"] == "0 4 * * *" and items["app1"]["keep"] == 10
    # remove one
    res = schedule.remove_schedule("app1")
    assert res["removed"] is True
    assert [s["app"] for s in schedule.list_schedules()] == ["app2"]
    # removing all clears the cron.d file
